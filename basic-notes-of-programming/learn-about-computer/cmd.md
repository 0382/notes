...
2. 通过手动指定相对路径或者绝对路径。
3. 在当前目录下的程序。这种情况似乎只在`cmd`中成立，一般的linux命令行是不支持的。
...
最后提一下，上述命令行输入的程序名中，有时候带了扩展名`.exe`，有时候没有带，这两种在Windows下都是可以的，但是在linux必须用全名（即包括扩展名，虽然linux下的可执行程序普遍不使用`.exe`扩展名）。
