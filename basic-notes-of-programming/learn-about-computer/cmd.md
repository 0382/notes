...
> 现在Windows大部分程序都支持使用`/`来作为文件夹分隔符，以前只能用`\`来分隔，造成很多人都习惯使用`\`。但是linux下只支持`/`。为了在各个系统下行为一致，还是建议统一使用`/`吧。不过`cmd`中有的时候是不支持的。
...
```
`git`是自己安装的程序`git.exe`，后面有三个参数，如果参数带空格，需要用双引号包裹起来。
